...
# To replace the word vector space
    model=KeyedVectors.load_word2vec_format('wiki.multi.en.vec') 
    model2=KeyedVectors.load_word2vec_format('wiki.multi.fr.vec') 
    ...
    for word, model in wv2.vocab.items():
        wordlist2.append(word)

    temp2.append(wordlist2)
    dictionary2 = corpora.Dictionary(temp2)
    dictionary2 = dictionary2.token2id

    for word, index in dictionary2.items():
        dictionary2[word] = dictionary2[word] + 2
    ...
    test_collect = []
    reader = pd.read_csv('task2_fr_test_participant.tsv', sep='\t')
    for line_id, line in enumerate(reader['tweet']):
        sentence = preprocess(line) + ' '
    ...
    for word, index in dictionary2.items():
        word_vector[word] = wv2[word]

    n_symbols = len(dictionary) + len(dictionary2)+1
    ...
    model = Sequential()
    model.add(Embedding(output_dim=300, input_dim=n_symbols + 1,
...
# set LSTM input dropout and recurrent dropout
    ...
    with open('task2_prediction_fr.csv','w') as f:
