...
def Logistic_regression(traindata, y_train_num, testdata, y_test_num):
    ...
    with open('task2_en_bow.csv','w') as f:
        for i in y_pred:
            f.write(i)
            f.write('\n')
...
X_train, y_train = import_data('task1_training.tsv')
X_test, y_test = import_data('task1_validation.tsv')
X_train.extend(X_test)
y_train.extend(y_test)
X_test = import_test_data('task1_test_participant.tsv')
